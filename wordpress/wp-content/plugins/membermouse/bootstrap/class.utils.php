 ...
 */
 ...
 {	
	...
	public static function getClientIPAddress()
	{
 	...
 	{
	 	if($status == '1') {
	    	return '<img src="'.MM_Utils::getImageUrl("bullet_green").'" title="Active" style="vertical-align:middle;" />';
	    }
	    else if($status == '0') {
	    	return '<img src="'.MM_Utils::getImageUrl("bullet_red").'" title="Inactive" style="vertical-align:middle;" />';
	    }
	    else
	    {
	    	return MM_NO_DATA;
	...
	{
		$cacheDir = ABSPATH."wp-content/plugins/".MM_PLUGIN_NAME."/com/membermouse/cache";
		...
		foreach($problemPlugins as $name=>$location)
		{
			if(in_array($location, $plugins))
			{
				if(!MM_Messages::isMessageHidden($location))
				{
					...
					MM_Messages::addError("<strong>MemberMouse Warning</strong>: The <em>{$name}</em> plugin is known to cause issues with MemberMouse. <a href='http://support.membermouse.com/customer/portal/articles/1148281-plugins-that-cause-problems' target='_blank'>Learn more</a> | <a href='{$hideWarningUrl}'>Hide this warning</a>");
				}
			}
		}
	}
