		...
		<tr>
			...
			<td>
				...
				<div id="mm-paid-bundle-settings" style="margin-top:5px; <?php if($bundle->isFree()) { echo "display:none;"; } ?>">
					...
					Products
					<img src="<?php echo MM_Utils::getImageUrl("information") ?>" style="vertical-align: middle" title="Paid bundles can have multiple products associated with them which allows you to offer different pricing for the same bundle Select one or more products below to associate with this bundle." /> 
					</span>
...
(Command key) + click</code>
		...
		<tr>
			<td nowrap>Default Membership <img src="<?php echo MM_Utils::getImageUrl('information'); ?>" style="vertical-align:middle;" title="If this bundle is the first thing a customer buys, the default membership level specified here will be applied to their account. If an existing member purchases this bundle, their membership level will remain unchanged. If you select 'use system default', the default system membership level will be used as defined on the Membership Levels screen. Only free membership levels can be used as the default membership level." /></td>
			<td>
		...
		<tr <?php echo ($provider_token == "default")?"style='display:none;'":""; ?>>
			<td nowrap>Short Name* <img src="<?php echo MM_Utils::getImageUrl('information'); ?>" style="vertical-align:middle;" title="Short names are passed to your email provider allowing you to segment your list based on which bundles a particular member has access to. You can name the short names anything you want. The best practice is to name it based on your bundle display name so that when you're looking to segment your list on your email provider you can readily associate the short names with the bundle."/></td>
			<td><input id="mm-short-name" type="text" maxlength="10" class="long-text" value="<?php echo htmlentities($bundle->getShortName(), ENT_QUOTES); ?>" <?php if ($bundle->hasSubscribers() && !empty($bundle->getShortName)) { echo "readonly='readonly'"; } ?> onChange="mmjs.checkShortNameAjax();"/>
		...
		<tr>
			<td>Protected Categories <img src="<?php echo MM_Utils::getImageUrl("information"); ?>" style="vertical-align:middle;" title="Select the WordPress categories that should be automatically protected by this bundle." /></td>
			<td>
...
<a href="javascript:mmjs.save();" class="mm-button blue">Save Bundle</a>
