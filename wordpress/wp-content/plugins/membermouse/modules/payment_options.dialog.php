 ...
 */
	...
	if(!$user->isValid())
	{
		...
		if(intval($p->userId) == MM_User::$NEW_USER_ID)
		{
			...
			$user = new MM_User();
			$user->setStatus(MM_Status::$PENDING);
...
{
	var subject = "Purchase '<?php echo $accessName; ?>'";
	var body = "You can purchase '<?php echo $accessName; ?>' using the following link: \n" + appendAffiliateParams(checkoutUrl);
	var mailToLink = "mailto:<? echo $user->getEmail(); ?>?subject=" + subject + "&body=" + encodeURIComponent(body);
...
<a href='javascript:<?php echo $compFunction; ?>' class="mm-button small">
<img src="<?php echo MM_Utils::getImageUrl("award_star_gold"); ?>" style="vertical-align:middle;" />
