...
<div class="mm-wrap">
    ...
    <div style='padding-top: 10px;'>
    	...
    	<div id='mm-custom'></div>
    		<input type='submit' class="mm-button small blue"  onclick="mmjs.clearCache();" name='view_schedule' value='View Schedule' /></form>
    	</div>
    	...
    	<input type='hidden' id='mm-expanded-rows-copy' name="mm_expanded_rows_copy" value='' />
    			...
    			<div id='mm-custom-copy'></div>
	    			<input type='submit' class="mm-button small"  name='copy_schedule' value='Copy Schedule' />
    	...
    	<?php } else if(count($generatedRows)<=0 && !is_null($selectedTypes)) {
    		...
    		?>
    			<div style="line-height:35px;">
    				<a onclick="mmjs.addAccessRights('<?php echo $type->id; ?>','<?php echo preg_replace("/[\"\']+/", "", $type->value); ?>');" class="mm-button small green">
    					<img src='<?php echo MM_Utils::getImageUrl("add"); ?>' style="vertical-align:middle;" /> 
    		...
    		<?php 
    			foreach($showRows as $type) {
    				...
    				<td  class='mm-dsm-header-access-types'>
    					...
    					<a title="Grant Access" onclick="mmjs.addAccessRights('<?php echo $type->id; ?>','<?php echo preg_replace("/[\"\']+/", "", $type->value); ?>');" class='mm-dsm-imagelink'><img class='mm-dsm-header-image-right' src='<?php echo MM_Utils::getImageUrl("add"); ?>' /></a> 
    					</td>
    				<?php 
    			}
    		?>
			    						...
			    						if($index == count($usedPages)-1){
						    				...
						    				<tr>
						    					...
						    					<td class='<?php echo $className; ?>'>
						    						<a style='cursor: pointer;' title="Edit Access" onclick="mmjs.updateAccessRightsDialog('<?php echo $id; ?>','<?php echo $currentType; ?>','<?php echo $pageId; ?>','<?php echo $day; ?>');"><img src='<?php echo MM_Utils::getImageUrl($image); ?>' /></a>
						    						<?php } ?>
