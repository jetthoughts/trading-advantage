...
var MM_1ClickPurchaseView = MM_Core.extend({
    ...
    updateOrderTotal:function()
    {
		...
		{
        	productPrice = parseFloat(jQuery("#mm_data_product_price").val().replace(/[^0-9-.]/g, ''));
		...
		{
        	shippingPrice = parseFloat(jQuery("#mm_data_shipping_price").val().replace(/[^0-9-.]/g, ''));
		...
		{
        	discount = parseFloat(jQuery("#mm_data_discount").val().replace(/[^0-9-.]/g, ''));
	...
	applyCoupon:function()
	{
		if(jQuery("#mm_field_coupon_code").length > 0 && jQuery("#mm_field_coupon_code").val().length > 0 && jQuery("#mm_field_coupon_code").val() != "")
		{
			...
			values.product_price = parseFloat(jQuery("#mm_data_product_price").val().replace(/[^0-9-.]/g, ''));
	...
	applyCouponCallback: function(response)
	{
		...
		else
		{
			...
			jQuery("#mm_label_coupon_error").show();
			oneclickpurchase_js.discountChangeHandler("$0.00");
		}
	}
});
